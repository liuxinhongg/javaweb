layui.config({
    ...
    commCallListInfo();
     ...
     //加载页面数据
    function commCallListInfo(pageData){
		
		if(pageData!=null){
			pageCurrent=pageData.curr;
		}
		$.ajax({
			url : "/code/pushmoney/getCommCallList.do?pageCurrent="+pageCurrent+"&pageSize="+pageSize,
			method: 'POST',
...
//异步刷新
function linksList(){
	$(".content1").html(renderDate(linksData.commCallPoList));
	...
	function renderDate(currData){
		...
		if(currData.length != 0){
			for(var i=0;i<currData.length;i++){
				...
				+'<td>'+currData[i].firstYRate+'</td>'
				+'<td>'+currData[i].secondYRate+'</td>'
				...
				dataHtml += '<td>';
		    	if(currData[i].status==1){
		    		dataHtml += '<a class="layui-btn layui-btn-mini links_edit" onclick="javascript:editCommCall(\''+currData[i].codeId+'\');"><i class="iconfont icon-edit"></i>编辑</a>'
				             +  '<a class="layui-btn layui-btn-danger layui-btn-mini links_removename" href="javascript:delCommCall(\''+currData[i].codeId+'\');">删除</a>';
		    	}
				dataHtml +='</td>'
				+'</tr>';
			}
...
function paged(obj){
	...
	$(".linksAdd_btn").click(function(){
		layer.open({
                    type: 2,
                    title: '新增话费提成标准',
                    content:['/code/pushmoney/toAddCommCall.do','no'] ,//不允许出现滚动条
                    area:['720px', '420px']
         });
	})

   window.delCommCall=function(data){
		
        layer.confirm('确定屏蔽此话费提成标准吗？',{icon:3, title:'提示信息'},function(index){
			
		    var index = layer.msg('信息蔽此中，请稍候',{icon: 16,time:false,shade:0.8});
			
			$.ajax({
		        url: "/code/pushmoney/deleteCommCalls.do",
		        data: {codeIds: data},
		        method: 'POST',
		        success: function (data) {
   ...
   window.editCommCall=function(data){
	   
	   layer.open({
           type: 2,
           title: '编辑话费提成标准',
           content:['/code/pushmoney/toEditCommCall.do?codeId=' + data,'no'] ,//不允许出现滚动条
           area:['720px', '420px']
      });
   }
...
//<!-----------------------------------------------------------分割线------------------------------------------------------------------>
   ...
   function positionConditionListInfo(){
		...
		$.ajax({
			url : "/code/position/getPositionConditionList.do?pageCurrent=1&pageSize=20&conditionType=4",
			method: 'POST',
			contentType: "application/json",
			success : function(data){
...
function linksList1(linksData1){
	...
	$(".linksAdd_btn1").click(function(){
		layer.open({
                   type: 2,
                   title: '新增话费提成标准条件',
                   content:['/code/position/toAddPositionCondition.do?conditionType=4','no'] ,//不允许出现滚动条
                   area:['720px', '520px']
        });
	})
  ...
  window.open=function(){
	  ...
	  window.location.href="/code/pushmoney.do";
  }
