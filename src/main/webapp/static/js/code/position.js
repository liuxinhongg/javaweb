layui.config({
    ...
    positionListInfo();
     ...
     //加载页面数据
    function positionListInfo(pageData){
		
		if(pageData!=null){
			pageCurrent=pageData.curr;
		}
		$.ajax({
			url : "/code/position/getPositionList.do?pageCurrent="+pageCurrent+"&pageSize="+pageSize,
			method: 'POST',
...
//异步刷新
function linksList(){
	$(".content1").html(renderDate(linksData.positionPoList));
	...
	function renderDate(currData){
		...
		if(currData.length != 0){
			for(var i=0;i<currData.length;i++){
				...
				+'<td>'+currData[i].positionComm+'</td>'
				+'<td>'+currData[i].positionDesc+'</td>'
				...
				dataHtml += '<td>';
		    	if(currData[i].status==1){
		    		dataHtml += '<a class="layui-btn layui-btn-mini links_edit" onclick="javascript:editPosition(\''+currData[i].id+'\');"><i class="iconfont icon-edit"></i>编辑</a>'
				             +  '<a class="layui-btn layui-btn-danger layui-btn-mini links_removename" href="javascript:delPosition(\''+currData[i].id+'\');">删除</a>';
		    	}
				dataHtml +='</td>'
				+'</tr>';
			}
...
function paged(obj){
	...
	$(".linksAdd_btn").click(function(){
		layer.open({
                    type: 2,
                    title: '新增岗位津贴',
                    content:['/code/position/toAddPosition.do','no'] ,//不允许出现滚动条
                    area:['720px', '480px']
         });
	})

   window.delPosition=function(data){
		
        layer.confirm('确定屏蔽此岗位及其津贴吗？',{icon:3, title:'提示信息'},function(index){
			
		    var index = layer.msg('信息蔽此中，请稍候',{icon: 16,time:false,shade:0.8});
			
			$.ajax({
		        url: "/code/position/deletePositions.do",
   ...
   window.editPosition=function(data){
	   
	   layer.open({
           type: 2,
           title: '编辑岗位津贴',
           content:['/code/position/toEditPosition.do?id=' + data,'no'] ,//不允许出现滚动条
           area:['720px', '480px']
      });
   }
...
//<!-----------------------------------------------------------分割线------------------------------------------------------------------>
   ...
   function positionConditionListInfo(){
		...
		$.ajax({
			url : "/code/position/getPositionConditionList.do?pageCurrent=1&pageSize=20&conditionType=1",
			method: 'POST',
			contentType: "application/json",
			success : function(data){
...
function linksList1(linksData1){
	...
	$(".linksAdd_btn1").click(function(){
		layer.open({
                   type: 2,
                   title: '新增岗位津贴条件',
                   content:['/code/position/toAddPositionCondition.do?conditionType=1','no'] ,//不允许出现滚动条
                   area:['720px', '520px']
        });
	})
