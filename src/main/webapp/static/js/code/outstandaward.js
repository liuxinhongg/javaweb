layui.config({
    ...
    commContributionListInfo();

     //加载页面数据
    function commContributionListInfo(pageData){
		
		if(pageData!=null){
			pageCurrent=pageData.curr;
		}
		$.ajax({
			url : "/code/outstandaward/getOutstandawardList.do?pageCurrent="+pageCurrent+"&pageSize="+pageSize,
			method: 'POST',
...
//异步刷新
function linksList(){
	$(".content1").html(renderDate(linksData.commContributionPoList));
	...
	function renderDate(currData){
		...
		if(currData.length != 0){
			for(var i=0;i<currData.length;i++){
		    	...
		    	+'<td>X ≥ '+currData[i].commMin + '</td>'
				+'<td>'+currData[i].commContribution+'</td>'
				...
				dataHtml += '<td>';
		    	if(currData[i].status==1){
		    		dataHtml += '<a class="layui-btn layui-btn-mini links_edit" onclick="javascript:editCommContribution(\''+currData[i].codeId+'\');"><i class="iconfont icon-edit"></i>编辑</a>'
				             +  '<a class="layui-btn layui-btn-danger layui-btn-mini links_removename" href="javascript:delCommContribution(\''+currData[i].codeId+'\');">删除</a>';
		    	}
				dataHtml +='</td>'
				+'</tr>';
			}
...
function paged(obj){
        ...
        var laypage = layui.laypage;
       	laypage({
			...
			jump : function(data,first){
				  pageData=data;
				  if (!first) {
					  commContributionListInfo(pageData);
                }
			}
		});
    });
	...
	$(".linksAdd_btn").click(function(){
		layer.open({
                    type: 2,
                    title: '新增杰出贡献奖标准',
                    content:['/code/outstandaward/toAddCommContribution.do','no'] ,//不允许出现滚动条
                    area:['720px', '420px']
         });
	})

   window.delCommContribution=function(data){
        ...
        layer.confirm('确定屏蔽此管理津贴标准吗？',{icon:3, title:'提示信息'},function(index){
			...
			$.ajax({
		        url: "/code/outstandaward/deleteCommContributions.do",
		        data: {codeIds: data},
		        method: 'POST',
		        success: function (data) {
   ...
   window.editCommContribution=function(data){
	   
	   layer.open({
           type: 2,
           title: '编辑杰出贡献奖标准',
           content:['/code/outstandaward/toEditCommContribution.do?codeId=' + data,'no'] ,//不允许出现滚动条
           area:['720px', '420px']
      });
   } 
...
//<!-----------------------------------------------------------分割线------------------------------------------------------------------>
   ...
   function positionConditionListInfo(){
		...
		$.ajax({
			url : "/code/position/getPositionConditionList.do?pageCurrent=1&pageSize=20&conditionType=7",
			method: 'POST',
			contentType: "application/json",
			success : function(data){
...
function linksList1(linksData1){
	...
	$(".linksAdd_btn1").click(function(){
		layer.open({
                   type: 2,
                   title: '新增杰出贡献奖领取条件',
                   content:['/code/position/toAddPositionCondition.do?conditionType=7','no'] ,//不允许出现滚动条
                   area:['720px', '520px']
        });
	})
  ...
  window.delPositionCondition=function(data){
       ...
       layer.confirm('确定屏蔽此杰出贡献奖条件信息吗？',{icon:3, title:'提示信息'},function(index){
