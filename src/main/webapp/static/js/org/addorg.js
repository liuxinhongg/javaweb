	...
	var form = layui.form(),
      ...
      form.on("submit(addLinks)",function(data){
      	...
      	var branchCity = $("input[name='branchCity']:checked").val();
			...
			$.ajax({
		        url: "/org/branch/addOrg.do",
		        data: {branchCity:branchCity,branchName: $.trim($("#branchName").val()),upId: $.trim($("#upId").val()),noId: $.trim($("#noId").val()),status: $.trim($("#status").val()),effectMonth: $.trim($("#effectMonth").val())},
		        method: 'POST',
