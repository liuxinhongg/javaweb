	...
	var form = layui.form(),
		...
		success : function(data){
			...
			if(data.length != 0){
				...
				for(var i=0;i<data.length;i++){
					...
					if(oldUpId == data[i].id){
						dataHtml+='<option value="'+data[i].id+'" selected>'+data[i].branchName+'</option>';
					}
					else{
      ...
      form.on("submit(addLinks)",function(data){
      	...
      	var index = layer.msg('提交中，请稍候',{icon: 16,time:false,shade:0.8});
			...
			$.ajax({
		        url: "/org/branch/editBatch.do",
		        data: {id:$.trim($("#id").val()),branchName: $.trim($("#branchName").val()),upId: $.trim($("#upId").val()),noId: $.trim($("#noId").val()),status: $.trim($("#status").val()),effectMonth: $.trim($("#effectMonth").val())},
		        method: 'POST',
		        success: function (data) {
